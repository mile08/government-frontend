class ContactPresenter < ContentItemPresenter
  ...
  def organisation
    @content_item["organisation"]
  end

  def organisation
    # The only organisation in here is HMRC, so call `.first` on the array.
    content_item["links"].fetch("organisations", []).map { |organisation| OpenStruct.new(organisation) }.first
  end

  def slug
    URI.parse(content_item["web_url"]).path.sub(%r{\A/}, "")
  end

  def related_links
    return [] unless content_item["links"]
    content_item.to_hash.dig("links", "related").to_a
  end
